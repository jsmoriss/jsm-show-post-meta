...
<p>`jsm_spm_view_cap ( 'manage_options' )` &mdash; The current user must have these capabilities to view the "Post Meta" metabox (default: 'manage_options' ).</p>

<p>`jsm_spm_post_type ( true, $post_type )` &mdash; Add the "Post Meta" metabox to the editing pages for this post type.</p>

<p>`jsm_spm_post_meta ( $post_meta, $post_obj )` &mdash; The post meta array (unserialized) retrieved for display in the metabox.</p>

<p>`jsm_spm_skip_keys ( $array )` &mdash; An array of key name prefixes to ignore (default: '_encloseme' ).</p>
